"""Medusa candidate v6: single Pallas kernel ragged Mamba 2 SSD on v6e.

Written from scratch from the semantic spec in ``taskset.json``. Not
derived from any forbidden source (AI-Hypercomputer/maxtext,
AI-Hypercomputer/JetStream, sgl-project/sglang-jax,
vllm-project/tpu-inference, jax.experimental.pallas.ops.tpu,
state-spaces/mamba) nor the active ``candidate.py``. v1–v4 archive
snapshots are read for hypothesis-formation only.

# v6 hypothesis: fuse all three SSD phases into one Pallas kernel

v4 ran Phase A (per-chunk intra-chunk + h_local) in Pallas, then Phase
B (cross-chunk scan over h_local → h_init_per_chunk) and Phase C
(per-token y_inter from h_init + C einsum) on the JAX side. The two
JAX-side phases produce and consume two intermediate tensors of shape
(NumChunks, H, P, N) f32 — ~322 MiB each on the gradual_mix case:

    h_local           : f32[NC, H, P, N]  (Pallas output)
    h_init_per_chunk  : f32[NC, H, P, N]  (Phase B output)

Plus the y_inter f32 intermediate (f32[NC, CHUNK, H, P]) and an
f32-cast version of C (f32[NC, CHUNK, H, N]) — together a half-GiB
plus of HBM round-trip *outside* the Pallas kernel.

v6 collapses all of that into a single ``pallas_call``:

  1. emit_pipeline still streams (x, dt_v, clog, B, C) per chunk
     (heavy I/O).
  2. ``h_init`` is a VMEM scratch buffer (``pltpu.VMEM((H, P, N),
     f32)``) that persists across emit_pipeline iterations. Zero-
     initialized once at the outer-kernel entry.
  3. The per-chunk body reads h_init *before* updating it, uses it
     to compute the y_inter contribution for the current chunk's
     tokens via one bf16 matmul ``h_init @ C^T``, and then updates
     h_init in place via ``h_init ← carry_factor[c] · h_init + h_local``.
  4. The kernel writes the *combined* y = (y_intra + y_inter).bf16
     for this chunk to HBM. No separate y_intra / h_local tensors
     are materialised in HBM.

Carry_factor[c, h] and inter_valid[c, t] are precomputed on the JAX
side (small tensors, hold in VMEM via ``memory_space=VMEM``) so the
kernel doesn't have to chase first_b / local_seg semantics in two
places.

# What this saves

On gradual_mix (NumChunks=77, H=128, P=64, N=128):

- h_local HBM write removed:               322 MiB
- h_local HBM read (was Phase B input):    322 MiB
- h_init_per_chunk HBM write:              322 MiB
- h_init_per_chunk HBM read (Phase C):     322 MiB
- y_inter f32 buffer write+read:           322 MiB × 2 = 644 MiB
- f32 cast of C for the einsum:            322 MiB

  total intermediate HBM traffic removed:  ~2.2 GiB

At v6e's 800 GB/s HBM bandwidth, eliminating ~2 GiB of intermediate
traffic releases up to ~2.5 ms — non-trivial against v4's ~10.4 ms
total. XLA may have fused some of this, so the realised win is
expected to be smaller (~5–15%).

# Algebra (matrix-dual, same as v3/v4)

Inside chunk c with h_init = h_init_scratch (carries from previous chunk):

    M[i, j, h] = exp(cum_log_a[i, h] - cum_log_a[j, h]) * dt_v[j, h]
                 * 1[same sub-seg(i, j)] * 1[j <= i]
    score[i, j, h] = sum_n C[i, h, n] * B[j, h, n]
    y_intra[i, h, p] = sum_j M[i, j, h] * score[i, j, h] * x[j, h, p]
    h_local[h, p, n] = sum_j M[CHUNK-1, j, h] * x[j, h, p] * B[j, h, n]
    decay[t, h]      = exp(cum_log_a[t, h])
    y_inter[t, h, p] = decay[t, h] * inter_valid[t]
                       * sum_n C[t, h, n] * h_init[h, p, n]
    y[t, h, p]       = (y_intra[t, h, p] + y_inter[t, h, p]).bf16

Update at end of chunk:
    h_init ← carry_factor[c] * h_init + h_local
where carry_factor[c, h] = exp(cum_log_a[c, CHUNK-1, h]) · 1[chunk has no reset].
"""
